...
GRIPPER_CHANNELS = {
    "/tars/motor/gripper2_feedback": "left_gripper",
    "/tars/motor/gripper4_feedback": "right_gripper",
}
...
def start_recording():
    global _recording, _cam_buffers, _gripper_buffers
...
async def handler(websocket):
    ...
    async for data in websocket:
        ...
        # =========================
        # CAMERA — parallel decode across channels
        # =========================
        cam_items = []
        for item in batch.camera_image_data:
            ...
            if ch not in CAM_CHANNELS:
                continue
            cam_items.append((ch, CAM_CHANNELS[ch], item.image.data))
        ...
        if cam_items:
            ...
            try:
                futures = [
                    ...
                    for ch, name, img_data in cam_items
                ]
            ...
            cam_updated = False
            for (ch, name, _), (img, jpeg_bytes) in zip(cam_items, results):
                if img is not None:
                    shared_buffer["cam"][name] = img
                    cam_updated = True

                    if rec and jpeg_bytes is not None:
                        _cam_buffers[name].append({"ts": recv_ts, "data": jpeg_bytes})
